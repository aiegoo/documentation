...
he script is meant to help calculating waypoints required to achieve overlapping coverage set by the clients. For example, where there are tall towers in the survey area, usual 70% coverage will result in errors when importing the cloud points to the px4d for mapping. So 80% overlapping coverage plus additional coverage may help to get the tight grid that may have been affected by the impeding obstacles. 
