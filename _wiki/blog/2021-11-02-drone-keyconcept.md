...
```diff 
@@좌/우 회전 비행의 경우 회전 속도를 높이지 않는 쪽의 회전을 그대로 유지한 채 
반토크를 이용하고자 하는 로터의 회전 속도만 증가시키는 경우 전체적인 양력 증가로 인한 상승 +비행을 하게 되므로 특정 방향의 회전을 증가 시키는 만큼 반대 방향의 회전은 상대적으로 감소시키되 증가한 회전과 감소한 회전으로 인한 양력은 정지 비행 시와 같이 멀티콥터의 중량만큼 양력을 발생 시켜야 한다. 
```
