...
#### 프로젝트 개요
우리는 Open Hardware Group의 CV32E40P 코어에 간단한 명령어 집합 아키텍처(ISA) 확장을 개발했습니다. 주요 목표는 Nexys-A7 FPGA 보드를 사용하여 에지에서 AI 추론 작업의 성능을 향상하는 것이었습니다.
...
3. **코어 RTL 수정**:
   - 가속기의 아키텍처를 지원하기 위해 코어 RTL(Register-Transfer Level)에 수정이 가해졌습니다.
   - 이러한 변경 사항은 주로 추가된 벡터 연산을 처리하는 데 중요한 다중 사이클 명령어를 지원하는 데 중점을 두었습니다.

4. **FPGA 개발 단계**:
   - **비트스트림 생성**: 코어 및 가속기를 포함한 RTL 설계가 비트스트림 파일로 합성되었습니다.
   - **FPGA 프로그래밍**: 비트스트림이 Nexys-A7 FPGA 보드에 로드되었습니다.
   - **디버깅**: FPGA에서 이진 파일을 디버깅하여 적절한 기능 및 성능 개선을 보장했습니다.

#### 성능 결과
확장 및 수정 결과 CV32E40P 코어에서 TinyMLPerf 벤치마크가 5배 이상 속도가 향상되었습니다. 이 중요한 성능 향상은 에지에서 저전력 AI 추론을 위해 사용자 정의 ISA 확장을 사용하는 가능성을 입증합니다.
...
#### 결론
이 프로젝트는 AI 추론 작업을 위해 성능을 크게 향상하는 CV32E40P 코어에 사용자 정의 ISA 확장을 개발 및 구현하는 방법을 보여줍니다. 결과는 특히 에너지 제약 환경에서 타겟 애플리케이션을 위해 특수 명령어로 오픈 소스 하드웨어 코어를 확장하는 것의 가능성과 이점을 강조합니다.
