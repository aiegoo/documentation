...
collection: wiki
categories: school
keywords: "class4 class3 dronetest fixedwing drone law class4 kotsa"
summary: "Thu, Dec 02, 21, class4 class3 dronetest fixedwing  Aviation law for safety, business, FIA, human factors, system, concept and weather"
excerpt_separator: <!--more-->
toc: true
published: false
parent: [[Wiki-Setting-Category]]
date: 2021-12-02T13:54:08 +0900
updated: 2021-12-02 13:54
---
...
My initial test score was averaged under 70% out of 40 questions. I have collected the contents at that level until recently, where my average score now is at mid 90s. Since then, the contents I have gathered from various sources including FAA official books are added so that I can refer to this page in the future.
...
.customtable-color {max-width: 100% !important;width: 100%;}
.customtable-color, tr:nth-child(1) {background-color:rgba(228 74 102 0.4) !important;}
</style>
...
| 자체 12kg초과 180kg 이하 | 무인비행장치 -> 무인 비행선 (길이 7m초과 20m 이하)
| 자체 12kg초과 이륙중량 25kg이하 | 무인비행장치 -> 무인비행기/무인헬기/무인멀티콥터 -> 인증 면제
| 이륙중량 25kg 초과 자체중량150kg 이하 | 무인비행장치 - 무인 비행기/무인헬기/무인 멀티 콥터
...
{: .customtable}
|---
| 엔진 : 글로우엔진 |  Glow Engine : 글로우플러그 알코올연료 2행정 4행정 연료고가 작은기체
...
{: .customtable}
|---
| 레이놀즈수: | 점성력에 대한 관성의 비, 층류와 난류를 구분하는 척도
| Re = Vx/y | Re: 레이놀즈수, V: 속도, x: 직경, y: 점성계수
...
[basic_airplane_construction](https://github.com/{{site.github_editme_path}}pdf/basic_rpas_construction.pdf)
...
- trim tabs [youtube](https://youtu.be/y4-3PvYNJrU)

![image](https://user-images.githubusercontent.com/42961200/145006394-1dea78b9-aa53-4616-84a8-2fc3f4a4fb40.png)

the movement of the elevator is opposite to the direction of movement of the elevator trim tab. [more_details](https://iammechanic423.tistory.com/4) or [source](https://www.boldmethod.com/learn-to-fly/systems/4-types-of-trim-tabs/)
...
그런데 왜 항공기의 탭이 엘리베이터/수평안정판과 같은 방향으로 움직이기를 필요로 하는가?
소형 항공기에서는 조종감을 높이고, 항공기의 피치를 지나치게 조절하는 것을 방지하는데 도움이 된다.
안티서보 탭의 가장 대표적 예시들 중 하나는 Piper Cheroke에 있다. 이것이 없다면, 비행기는 상승하강하는 것이
훨씬 쉬울 것이다. 하지만 항공기를 과도 조종하기 쉬워짐과 동시에, 아마도 기체에 스트레스를 줄 것이다.

![tail_tab](images/drone-resource-wiki/tail_tab.jpeg)
![trim_tab](images/drone-resource-wiki/trim_tab.gif)


![balance_tab](images/drone-resource-wiki/balance_tab.jpeg)
(⇧) 어떤 항공기는 특히 고속에서 매우 무거운 제어 하중을 가지고 있다. 이러한 경우 밸런스 탭이 유용하다.
밸런스 탭은 트림 탭처럼 보이지만 하나의 중요한 차이점이 있는데 밸런스 탭은 제어 표면 링키지에 부착되어 있기
때문에 제어 표면이 한 방향으로 이동하면 밸런스 탭은 반대 방향으로 이동한다.
밸런스 탭을 반대 방향으로 이동하면 요크에 대한 제어 부하가 크게 감소하여 비행기 조종감을 높이게 된다.
이 밸런스 탭의 본래의 목적이 무엇이였는가? 요크에 대한 제어 부하를 감소시켜 조종사가 큰 조종력을 필요로 하지 않고서도 항공기를 조종하는 것이다.
![antiservo_tab](images/drone-resource-wiki/antiservo_tab.jpeg)

안티서보 탭은 밸런스 탭과 비슷하지만 반대 방향으로 이동한다. 예를 들어, 조종사가 엘리베이터나 수평안정판이
위로 움직일 때, 안티서보 탭은 같은 방향으로 움직인다.

그런데 왜 항공기의 탭이 엘리베이터/수평안정판과 같은 방향으로 움직이기를 필요로 하는가?
소형 항공기에서는 조종감을 높이고, 항공기의 피치를 지나치게 조절하는 것을 방지하는데 도움이 된다.
안티서보 탭의 가장 대표적 예시들 중 하나는 Piper Cheroke에 있다. 이것이 없다면, 비행기는 상승하강하는 것이 훨씬 쉬울 것이다. 하지만 항공기를 조종하기 쉬워짐과 동시에, 아마도 기체에 과도한 스트레스를 줄 것이다.
...
|정압(P) + 동압(q) = 전압(Pt) | Pt = Ps + 1/2*밀도g*V^2*S*Ct ![전압](https://raw.githubusercontent.com/wiki/aiegoo/drone-eggs/images/LCL.jpg?token=AKHYSMEV7VUNV6F2FYU6UK3BY2BPM)  |  ![dthumb-phinf pstatic](https://user-images.githubusercontent.com/42961200/146508993-813cdac7-6688-42bb-bd2d-8dd1c33da724.jpg)
...
![dthumb-phinf pstatic](https://user-images.githubusercontent.com/42961200/146509263-5a98045e-6380-4618-9aaf-9f8fe4bf636f.jpg)

P : 압력   p : 밀도  v : 속도   g : 중력가속도    h : 높이

베르누이의 정리 입니다.



이해하기 쉽게 좌변 분자 분모에 부피 V 를 곱해봅시다.

![dthumb-phinf pstatic](https://user-images.githubusercontent.com/42961200/146509201-dc517d3a-0deb-493a-8f6f-7f189b3e29dd.jpg)

밀도 p 와 부피 V 를 곱했으니 질량 m 이 되겠죠?



그럼 일정 부피의 유체(~/V)가 갖는 전압은 PV (일 에너지) + 1/2 mv^2 (운동에너지) + mgh (위치에너지) ​


로 일정하게 됩니다.    열역학 1법칙 에너지 보존의 법칙이 생각납니다.
...
정수압을 빼고 정압과 동압을 합쳐서 정체압 이라고 합니다. (베르누이의 법칙을 이용하는 데, 정수압은



의미가 없는 경우가 많기 때문에 정압과 동압을 합쳐서 전압이라고 부르는 경우도 많은 것 같습니다.)



여기서 정압은 우리가 흔히말하는 압력입니다.


**피토관**
피토관은 정압과 동압의 합이 일정하다는 법칙을 이용해서 속도를 측정합니다.


![dthumb-phinf pstatic](https://user-images.githubusercontent.com/42961200/146509036-7116bd37-d65a-4062-bbb8-0bae87766b57.jpg)

이 식을 속도(v) 에 대하여 정리하면,

![dthumb-phinf pstatic](https://user-images.githubusercontent.com/42961200/146508993-813cdac7-6688-42bb-bd2d-8dd1c33da724.jpg) ∵ P : 정압

정체압과 정압 그리고 유체의 밀도를 알 수 있으면 속도를 구할 수 있습니다!


유체의 밀도는 분석을 한다면 알 수 있겠고, 정체압과 정압의 차는 피토관을 통해 알 수 있습니다.

![bernoulli](https://user-images.githubusercontent.com/42961200/146508666-2f056337-ce8a-4542-b22b-8b1417dba0e9.jpg)

위와 같이 피토관이 있고, 화살표 방향으로 유체가 흘러갑니다.


정압은 ①번 ②번 관 모두에 힘을 가합니다.

정압은 우리가 흔히 말하는 압력이고 ,  압력은 모든 면에 동일하게 작용하기 때문이죠.

동압은 유체의 진행 방향과 수평인 ②번 관에만 힘을 가하고, 진행방향과 수직인 ①번 관에는 힘을 가하지 않습니다.

그래서  두 관의 압력차이는 정체압 - 정압, 즉 동압이 됩니다.
...
|---
|초속 0.3~1.5m :실바람 풍력 계급의 1등급| 가장 여린바람. 연기가 풀려서 오르고, 해면은 물고기 비늘 모양의 잔물결이 일어남.
|초속 1.6~3.3m :남실바람 풍력 계급의 제2등급| 바람이 얼굴에 느껴지고 나뭇잎이 살랑거리며 풍향계가 움직이고, 해면의 잔물결이 뚜렷이 일어남.
| 초속 3.4~5.4m :산들바람 풍력 계급 3등급| 나뭇잎과 잔가지가 일정하게 흔들리고 깃발이 가볍게 나부끼며, 해면은 군데군데 흰 물결이 생김.
| 초속 5.5~7.9m :건들바람 풍력 계급 4등급| 먼지가 일고 종잇장이 난다.
...
5. 상층운 : 6,000m 이상 상공에서 형성된다. 공기가 차고, 건조, 빙정으로 형성, 얇은 층을 이룬다.
...
⑴ 공항 등화시설 : 비행장 등화라고도 하며, 공항 또는 주변에 설치된 시설로서 이·착륙 또는 지상주행을 위해 사용되는 시설이다.
이러한 시설은 용도에 따라 비행장등대, 보조비행장등대, 선회등, 진입등, 진입각 지시등, 활주로등, 활주로말단등, 비상용 활주로등, 활주로 말단 연장등, 접지대등, 활주로 거리등, 경계등, 경계유도등, 유도로등, 유도안내등, 착륙방향지시등, 이륙목표등, 활주로 중심선등, 활주로 유도등, 활주로 말단 식별등, 활주로 진입주의등, 정지선등, 계류장 조명등, 통과선등, 진입구역등, 정지로등, 활주로종단등, 비행장 명칭 표시등, 주기장 식별표시등, 주기장 안내등, 탑승교유도집현 등으로 분류된다.

⑵ 항공로 등화시설 : 항공등대라고도 하며, 항공로를 항해하는 항공기에게 항공로상의 중요 지점을 알리는 시설이다.
...
③ 신호항공등대(signalling aeronautical beacon) : 항공로상 또는 부근의 특정지역을 가리키는 것. 모스부호를 점멸시킨다. 육상에서는 녹색, 수상에서는 황색, 이
밖에 적색을 사용한다.
④ 위험항공등대(hazard beacon) : 항공장애지구를 알려 주며, 위험지역을 적색등으로 둘러싸고 있
다.

⑶ 항공장애물 등화시설:항공장애등이라고도 하며, 항공기의 안전항행을 현저히 방해할 우려가 있는 장애물의 존재 ·위치 ·윤곽 등을 표시하는 시설이다.
항공로상의 높이 60 m 이상 되는 건조물 또는 비행장 주변의 진입로 이외의 곳에 있는 45 m 이상의 건조물 등에 설치하며, 등화의 종류로는 고광도의 것과 저광도의 것 등 2가지가 있다.
고광도는 적색의 깜박등이며,저광도는 적색의 부동광(不動光)이다.

⑷ 항공기 등화시설:항공기의 내부 또는 외부에 설치하는 등화시설의 총칭으로, 흔히 비행등 또는 항공등이라고 한다. 항공기가 야간에 공중과 지상 또는 수상에서 항행하는 경우에 현위치를 나타내기 위하여 충돌방지등 ·우현등 ·좌현등 ·미등을 설치한다.
일반적으로 섬광방식(閃光方式)으로 된 등화를 사용하지만 때로는 부동광(不動光)의 등화도 사용되며, 그 등의 빛깔로 비행방향을 나타낸다
