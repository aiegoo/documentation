...
.tabs > li {padding:0.75rem; width:50%; float:left; text-align:center; left: -16px; }
.wrap > div {width:66%; top:calc(11.2% + 4rem); position:absolute; height:calc(77.4% - 3rem); left:2%; }
.conts {width:69%; height:94%; overflow-y:auto; position:absolute;     font-size: 11px; margin-top: 15px; }
.daily {display:block;}
.weekly {display:none;}
.daily > :first-child {padding:0em; text-align:center;}
.daily #wt_icon {width:30%;}
.daily .tit {text-align:right; display:inline-block; width:30%; padding:0em;}
