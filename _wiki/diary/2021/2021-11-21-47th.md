...
.tabs {border-bottom:1px solid #999; position:absolute; top:11.2%; left:7.13%; width:76%;}
.tabs > li {padding:0.75rem; width:50%; float:left; text-align:center;}
.wrap > div {width:66%; top:calc(11.2% + 3rem); position:absolute; height:calc(77.4% - 3rem); left:7.13%; }
.conts {width:100%; height:100%; overflow-y:auto; position:absolute;     font-size: 10px; margin-top: 28px; }
.daily {display:block;}
.weekly {display:none;}
