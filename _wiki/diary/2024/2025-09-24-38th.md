...
```
![Img_136_8-book_reader_ReadEra.png](https://github.com/user-attachments/assets/e8a907d9-7aa4-41bc-8caf-f8e158d6407e)
...
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| **RASA (NLU/Core)**          | Handles rule-based chatbot logic, intent recognition, and dialogue flow. Can call out to action servers or APIs.                                |
| **Ollama LLM**               | Runs Mistral/Phi/LLama3 locally and serves as a backend for open-ended QA, summaries, code generation, etc.                                     |
| **LangChain / LlamaIndex**   | Adds document memory via RAG—splits, embeds, and indexes knowledge sources; retrieves chunks for LLM to answer user questions grounded in data. |
| **Vector DB (FAISS/Chroma)** | Stores embeddings of PDFs, websites, and document corpora for similarity search.                                                                |
| **MiniGPT-4 / LLaVA**        | Allow users to upload images or charts and ask questions about them (e.g., "What is the y-axis value in this graph?").                          |
| **Whisper ASR / TTS**        | (Optional) Adds voice interface—transcribes user speech and converts system responses back to speech.                                           |
