...
1. Open VSCode and add the PX4 source code: 
   - Select *Open folder ...* option on the welcome page (or using the menu: **File > Open Folder**):
     ![Open Folder](images/drones/toolchain/vscode/welcome_open_folder.jpg)
...
1. Press **Install All** on the *This workspace has extension recommendations* prompt (this will appear on the bottom right of the IDE).
   ![Install extensions](images/drones/toolchain/vscode/prompt_install_extensions.jpg)

   VSCode will open the *Extensions* panel on the left hand side so you can watch the progress of installation.

   ![PX4 loaded into VSCode Explorer](images/drones/toolchain/vscode/installing_extensions.jpg)
...
1. Select your build target ("cmake build config"):
   - The current *cmake build target* is shown on the blue *config* bar at the bottom (if this is already your desired target, skip to next step).
     ![Select Cmake build target](images/drones/toolchain/vscode/cmake_build_config.jpg)
   ...
   - *Cmake* will then configure your project (see notification in bottom right).
     ![Cmake config project](images/drones/toolchain/vscode/cmake_configuring_project.jpg)
   - Wait until configuration completes.
     ...
     ![Cmake config project](images/drones/toolchain/vscode/cmake_configuring_project_done.jpg).
1. You can then kick off a build from the config bar (select either **Build** or **Debug**).
   ![Run debug or build](images/drones/toolchain/vscode/run_debug_build.jpg)
...
1. Select the debug icon on the sidebar (marked in red) to display the debug panel.
   ![Run debug](images/drones/toolchain/vscode/vscode_debug.jpg)
...
![Image showing hardware targets with options for the different probes](images/drones/toolchain/vscode/vscode_hardware_debugging_options.png)
...
![IntelliSense](images/drones/toolchain/vscode/vscode_intellisense.jpg)
